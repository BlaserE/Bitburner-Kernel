...
export async function main(ns: NS): Promise<void> {
    ...
    const localMajorVer: string = vData.local.split('.')[0];
    const remoteMajorVer: string = vData.remote.split('.')[0];

    if (localMajorVer !== remoteMajorVer && vData.local !== "0.0.0") {
        const result = await ns.prompt(
            `WARNING: Major version different (Remote version: ${vData.remote} | Local version: ${vData.local}). Proceed anyways?`,
            {type:"boolean"}
        );
        if (!result) {
            return;
...
async function TryPullFile (ns:NS, url:string, path:string, remoteHash:string):Promise<boolean> {
    ...
    const tmpPath:string = `/tmp/${fileName}`;
