import {IProcess, IProcessTable, ProcessSpec, RestartPolicy} from "../lib/process";
import {ProcessInfo} from "@ns";
import {Priority, ProcessState} from "../lib/values";


class ProcessTable implements IProcessTable {
    ...
    public add(spec: ProcessSpec): IProcess {
        let policy: Partial<RestartPolicy>;
    ...
    public get(pid: number): IProcess | undefined {
     ...
     */
    public list(filter?: Partial<IProcess>): IProcess[] {
        if (!filter || Object.keys(filter).length === 0) {
            return [...this._processes]; // Returning a shallow copy prevents external mutation
        }

        const filterEntries = Object.entries(filter) as [keyof IProcess, unknown][];
        ...
        return this._processes.filter(process => {
            ...
            return filterEntries.every(([key, expectedValue]) => {
                ...
                if (Array.isArray(expectedValue)) {
                    ...
                    if (!Array.isArray(processValue)) return false;
